...
- **Lark Open API** — Document content fetching via `docx/v1/documents/{document_id}/raw_content`. Authentication via tenant access token (`auth/v3/tenant_access_token/internal`). Supports Lark Docs and Wiki URLs from both `larksuite.com` and `feishu.cn` domains.
...
| `LARK_BASE_URL` | ✗ (default `https://open.larksuite.com`) | Lark Open API base URL (use `https://open.feishu.cn` for Feishu) |
